...
namespace MiscPatcherUtil
{
	public static class PatchField
	{
		...
		public static void PatchRecordFlags<TMajor, TMajorGetter>(this TMajor patched, HashSet<TMajorGetter> vanillas, TMajorGetter modded)
			where TMajor : SkyrimMajorRecord, TMajorGetter
			where TMajorGetter : ISkyrimMajorRecordGetter
		{
			var patchedValue = patched.MajorRecordFlagsRaw;
			var moddedValue = modded.MajorRecordFlagsRaw;
			var vanillaValues = vanillas.Select(x => x.MajorRecordFlagsRaw);

			patched.MajorRecordFlagsRaw = Util.GetPatchedFlags(patchedValue, vanillaValues, moddedValue);
		}

		#endregion

		#region Image space specific
		public static void PatchHdr(this ImageSpace patched, HashSet<IImageSpaceGetter> vanillas, IImageSpaceGetter modded)
		{
		...
		public static void PatchCinematic(this ImageSpace patched, HashSet<IImageSpaceGetter> vanillas, IImageSpaceGetter modded)
		{
		...
		public static void PatchTint(this ImageSpace patched, HashSet<IImageSpaceGetter> vanillas, IImageSpaceGetter modded)
		{
		...
		public static void PatchFlags(this Light patched, HashSet<ILightGetter> vanillas, ILightGetter modded)
		{
			var patchedValue = (int)patched.Flags;
			var moddedValue = (int)modded.Flags;
			var vanillaValues = vanillas.Select(x => (int)x.Flags);

			patched.Flags = (Light.Flag)Util.GetPatchedFlags(patchedValue, vanillaValues, moddedValue);
		}
		#endregion

		#region Cell specific
		public static void PatchFlags(this Cell patched, HashSet<ICellGetter> vanillas, ICellGetter modded)
		{
			var patchedValue = (int)patched.Flags;
			var moddedValue = (int)modded.Flags;
			var vanillaValues = vanillas.Select(x => (int)x.Flags);

			patched.Flags = (Cell.Flag)Util.GetPatchedFlags(patchedValue, vanillaValues, moddedValue);
		}

		#endregion
		#region Placed object specific
		public static void PatchPrimitive(this PlacedObject patched, HashSet<IPlacedObjectGetter> vanillas, IPlacedObjectGetter modded)
		{
			var patchedValue = patched.Primitive;
			var moddedValue = modded.Primitive;
			var vanillaValues = vanillas.Select(x => x.Primitive);

			var patchedBounds = patched.Primitive?.Bounds;
			var moddedBounds = modded.Primitive?.Bounds;
			var vanillaBounds = vanillas.Select(x => x.Primitive?.Bounds);
			patchedBounds = Util.PatchReverted(patchedBounds, vanillaBounds, moddedBounds, 0.0001f);

			if (patchedValue == null || moddedValue == null)
				patched.Primitive = moddedValue?.DeepCopy();
			else
				patchedValue.Bounds = patchedBounds!.Value;
		}
		public static void PatchLightData(this PlacedObject patched, HashSet<IPlacedObjectGetter> vanillas, IPlacedObjectGetter modded)
		{
